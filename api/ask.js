...
export default async function handler(req, res) {
  ...
  const lengthInstruction = mode === "long"
    ...
    : isGame
    ? "This answer will be spoken aloud by a character in a small game, so keep it to 2-4 plain, natural sentences a person would actually say — no lists, headings or markdown. Never use the bare words \"meat\" or \"leather\" (they are product language): say \"flesh\" or \"animal flesh\", e.g. \"lab-grown flesh\", and \"skin\" or \"animal skin\" instead of leather; if you must echo either word, wrap it in quotation marks. Keep the same JSON shape as always."
  ...
  try {
    ...
    if (isGame) { // product language guard: "meat" and "leather" only ever appear in quotation marks
      const q = (t) => t.replace(/(^|[^"“])\b(meat|leather)\b(?![”"])/gi, (m, pre, w) => `${pre}“${w}”`);
